...
(function(jsod){
    ...
    Merge.mergeRecords = function(recordA, recordB, deltaNodeA, deltaNodeB, config, localConflicts, parentPath)
    {
        ...
        if(opA != opB)
        {
            Util.reportConflict(localConflicts, "diffOp", recordA, recordB);
        }
        else if(opA == opB && !_.isEqual(valA, valB))
        {
            Util.reportConflict(localConflicts, "sameOpDiffVal", recordA, recordB);
        } else {
     ...
     * @param {*} [metaData]
     */
    Util.reportConflict = function(conflicts, conflictType, dataA, dataB, metaData)
    {
        let conflictReport = {
