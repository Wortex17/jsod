...
(function(jsod){
    ...
    Merge.mergeRecords = function(recordA, recordB, deltaNodeA, deltaNodeB, config, localConflicts, parentPath)
    {
        ...
        if(opA != opB)
        {
            Util.reportLocalConflict(localConflicts, jsod.Attributes.ConflictType.RECORD_DIFF_OPERATION, recordA, recordB);
        }
        else if(opA == opB && !_.isEqual(valA, valB))
        {
            Util.reportLocalConflict(localConflicts, jsod.Attributes.ConflictType.RECORD_DIFF_VALUE, recordA, recordB);
        } else {
     ...
     * @type {DiffConfiguration}
     */
    Presets.Configs.TreeDiffPure = _.extend(Presets.Configs, {
        compareHandlers: [
            ComparisonHandlers.compareAsTree,
            ComparisonHandlers.compareAsValue
        ]
     ...
     * @type {DiffConfiguration}
     */
    Presets.Configs.UnorderedValuelistDiffPure = _.extend(Presets.Configs, {
        compareHandlers: [
            ComparisonHandlers.compareAsUnorderedValuelist,
            ComparisonHandlers.compareAsValue
        ]
     ...
     * @type {DiffConfiguration}
     */
    Presets.Configs.OrderedListDiffPure = _.extend(Presets.Configs, {
        compareHandlers: [
            ComparisonHandlers.compareAsOrderedList,
            ComparisonHandlers.compareAsValue
        ]
     ...
     * @type {DiffConfiguration}
     */
    Presets.Configs.ValueDiffPure = _.extend(Presets.Configs, {
        compareHandlers: [
            ComparisonHandlers.compareAsValue
        ]
    });
